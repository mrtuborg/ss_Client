...
void cmd::dbgPrint()
{
  if (verbose_level) printf("cmd_length=%d\n",getCmdLength());
  printf("Функция №%d", func_id);
  if (verbose_level) printf("func_paramsLength:%lu\n",func_paramsLength);
...
void* cmd::get_func_paramsPtr(DWORD offset)
{
...
bool cmd::checkSign()
{
    ...
    cmd test_cmd;
    WORD test_sign;
    test_cmd.encode(this->func_id, this->func_paramsLength, (BYTE*)func_params);
    test_cmd.makeSign();

...
WORD Time_adj[1]={0};
BYTE zeroValue=0;
char tittle[255]={"Тонкий клиент управления службой контроля СУ электропитания"};
errType menuInit(menuString **menu, udpAction *sndAct, udpAction *rcvAct)
{
    ...
    menu[1]=new menuString(1,"Запрос включения питания всех систем КОС", 0, 1, sndAct, rcvAct);
    menu[2]=new menuString(2,"Запрос выключения питания всех систем КОС", 0, 1, sndAct, rcvAct);
    menu[3]=new menuString(3,"Запрос включения питания всех подсистем одной системы КОС", 1, 1, sndAct, rcvAct);
    menu[4]=new menuString(4,"Запрос выключения питания всех подсистем одной системы КОС", 1, 1, sndAct, rcvAct);
    menu[5]=new menuString(5,"Запрос включения питания подсистемы", 1, 1, sndAct, rcvAct);
    menu[6]=new menuString(6,"Запрос выключения питания подсистемы", 1, 1, sndAct, rcvAct);
    menu[7]=new menuString(7,"Запрос обобщённого статуса состояния электропитания питания всех систем КОС", 0, 2, sndAct, rcvAct);
    menu[8]=new menuString(8,"Запрос обобщённого статуса состояния электропитания питания одной системы КО", 1, 2, sndAct, rcvAct);
    menu[9]=new menuString(9,"Запрос статуса состояния электропитания питания подсистемы", 1, 2, sndAct, rcvAct);
    ...
    menu[3]->paramsConstruct(0, "Идентификатор системы КОС", type_WORD, &zeroValue);
    menu[4]->paramsConstruct(0, "Идентификатор системы КОС", type_WORD, &zeroValue);
    menu[5]->paramsConstruct(0, "Идентификатор подсистемы КОС", type_WORD, &zeroValue);
    menu[6]->paramsConstruct(0, "Идентификатор подсистемы КОС", type_WORD, &zeroValue);
    menu[8]->paramsConstruct(0, "Идентификатор системы КОС", type_WORD, &zeroValue);
    menu[9]->paramsConstruct(0, "Идентификатор подсистемы КОС", type_WORD, &zeroValue);
    ...
    menu[7]->resultsConstruct(1, "Статус состояния электропитания КОС в целом", type_BYTE, &zeroValue);
    menu[8]->resultsConstruct(0, "Квитанция исполнения", type_ERRTYPE, &coord_type);
    menu[8]->resultsConstruct(1, "Статус состояния электропитания системы КОС", type_BYTE, &zeroValue);
    menu[9]->resultsConstruct(0, "Квитанция исполнения", type_ERRTYPE, &coord_type);
    menu[9]->resultsConstruct(1, "Статус состояния электропитания подсистемы КОС", type_BYTE, &zeroValue);

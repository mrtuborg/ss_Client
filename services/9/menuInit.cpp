...
WORD Time_adj[1]={0};
BYTE zeroValue=0;
char tittle[255]={"Тонкий клиент управления службой контроля системы адаптивной оптики"};
errType menuInit(menuString **menu, udpAction *sndAct, udpAction *rcvAct)
{
    ...
    menu[1]=new menuString(1,"Запрос включения режима измерения при помощи системы адаптивной оптики", 0, 1, sndAct, rcvAct);
    menu[2]=new menuString(2,"Запрос выключения режима измерения при помощи системы адаптивной оптики", 0, 1, sndAct, rcvAct);
